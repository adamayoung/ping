...
public actor SiteSwiftDataDataSource: SiteDataSource {
    ...
    public func fetchAll() async throws -> [PingDomain.Site] {
        ...
        let sites = siteModels.compactMap(PingDomain.Site.init)
    ...
    public func update(_ site: PingDomain.Site) async throws {

    }

    public func `delete`(_ id: PingDomain.Site.ID) async throws {
        let predicate = #Predicate<Site> { $0.id == id }
